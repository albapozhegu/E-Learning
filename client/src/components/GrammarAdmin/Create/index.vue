...
import GrammarContentForm from "./GrammarContentForm.vue";
import grammarApi from "@/apis/grammarApi.js";
export default {
  name: "GrammarAdminCreateForm",
  data() {
    return {
      form: {
        Script: "",
  ...
  methods: {
    async handleSubmit() {
      ...
      try {
        const apiRes = await grammarApi.postGrammar(this.form);

        if (apiRes.status === 200) {
          alert("Created grammar successfully", "success")
